...
Write-Host "${BOLD}╔══════════════════════════════════════════════════╗${NC}"
Write-Host "${BOLD}║      DeepSeek Zoograf Client — Windows Setup    ║${NC}"
...
if ($pyVersion -match "3\.(1[0-9]|[2-9]\d)\.") {
    Write-Host "${GREEN}[✓]${NC} Python detected: $pyVersion"
} else {
...
Write-Host "${GREEN}[✓]${NC} All dependencies installed"
...
if (-not (Test-Path ".env")) {
    ...
    Write-Host "${GREEN}[✓]${NC} .env already exists"
}
...
$result = & $python -c "import textual; import openai; import rich; print('[✓] All imports OK')" 2>&1
Write-Host "  $result"
...
Write-Host "${GREEN}${BOLD}║     ${CYAN}run.bat${NC}                              ${GREEN}${BOLD}║${NC}"
Write-Host "${GREEN}${BOLD}║     or                                           ║${NC}"
Write-Host "${GREEN}${BOLD}║     ${CYAN}venv\Scripts\activate && python main.py${NC}  ${GREEN}${BOLD}║${NC}"
Write-Host "${GREEN}${BOLD}║                                                 ║${NC}"
Write-Host "${GREEN}${BOLD}╚══════════════════════════════════════════════════╝${NC}"
Write-Host ""
